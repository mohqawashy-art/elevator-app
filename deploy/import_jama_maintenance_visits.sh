#!/usr/bin/env bash
# استيراد زيارات الصيانة لجما من Excel
# Usage (GCP SSH):
#   cd ~/liftcore/jama-elevator-app && git pull origin main
#   bash deploy/import_jama_maintenance_visits.sh --dry-run
...
echo "==> Import maintenance visits into Jama"
echo "    DB:   $DB_FILE"
echo "    File: $XLSX"
python scripts/import_maintenance_visits_xlsx.py "${ARGS[@]}"

if [ "$DRY" != "1" ]; then
  sudo systemctl restart liftcore-jama 2>/dev/null || true
  echo "==> Done. Refresh https://jama.liftcoreapp.com/maintenance-visits"
fi
