#!/usr/bin/env bash
# تحديث عناوين عملاء جما من Excel + إحداثيات الخريطة
# Usage (GCP SSH):
#   cd ~/liftcore/jama-elevator-app && git pull origin main
#   bash deploy/import_jama_client_addresses.sh
#   bash deploy/import_jama_client_addresses.sh --dry-run
...
XLSX="${XLSX:-$JAMA_DIR/deploy/data/jama_clients_13_6_2026.xlsx}"
DRY="${DRY:-0}"
NO_GEO="${NO_GEO:-0}"
...
if [ ! -f "$XLSX" ]; then
  ...
  echo "Ensure deploy/data/jama_clients_13_6_2026.xlsx exists (git pull)."
  exit 1
fi
...
echo "==> Import client addresses into Jama"
echo "    DB:   $DB_FILE"
echo "    File: $XLSX"
python scripts/import_client_addresses_xlsx.py "${ARGS[@]}"
