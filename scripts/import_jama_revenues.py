...
CONTRACT_REVENUE_TYPES = frozenset({
    'تجديد عقد', 'دفعة عقد', 'الدفعات المستحقة', 'عقد جديد',
    'عقد تركيب', 'عقد تحديث', 'ضمان', 'عقد صيانة',
})
...
def _normalize_revenue_type(raw: str) -> str:
    ...
    if s == 'زيارة':
        return 'أعمال إضافية'
    if s in (
        'تجديد عقد', 'دفعة عقد', 'الدفعات المستحقة', 'عقد جديد', 'قطع غيار',
        'عقد تركيب', 'عقد تحديث', 'أعمال إضافية', 'أخرى', 'عقد صيانة',
    ):
        return s
    if 'مستحق' in s and 'دفعات' in s:
        return 'دفعة عقد'
    if s in ('دفعة عقد', 'دفعات مستحقة'):
        return 'دفعة عقد'
    if 'تركيب' in s:
        return 'عقد تركيب'
    if 'تحديث' in s:
