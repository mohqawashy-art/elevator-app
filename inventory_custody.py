...
# صرف لفني = استهلاك مباشر (لا عهدة). العهدة فقط من «صرف عهدة للفني» لقطع الغيار.
CUSTODY_ISSUE_TYPES = ('صرف عهدة للفني',)
...
_CONSUMABLE_CATEGORY_KEYS = frozenset({
    'مستهلكات',
    'مستهلكللصيانة',
    'consumables',
})
...
def _category_key(category: str | None) -> str:
    return (category or '').strip().replace(' ', '').replace('ـ', '').lower()


def item_eligible_for_custody(item: InventoryItem | None) -> bool:
    """المستهلكات/مواد الصيانة الاستهلاكية لا تُسجَّل عهدة."""
    if not item:
        return True
    ...
    if key in _CONSUMABLE_CATEGORY_KEYS:
        return False
    if 'مستهلك' in cat:
        return False
    return True
...
def movement_counts_as_custody_issue(movement_type: str, item: InventoryItem | None = None) -> bool:
    mt = (movement_type or '').strip()
    if mt not in CUSTODY_ISSUE_TYPES:
        return False
    return item_eligible_for_custody(item)
...
def build_technician_custody_snapshot() -> dict:
    ...
    items = tenant_query(InventoryItem).all()
    item_meta = {
        i.id: {'code': i.code or '', 'name': i.name or '', 'category': i.category or ''}
        for i in items
    }
    ...
    for m in tenant_query(StockMovement).filter(
        ...
        item = item_by_id.get(int(m.item_id))
