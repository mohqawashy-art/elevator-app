(function (global) {
  ...
  var VIEW_UPLOAD_PATH = /\/view\/upload\//i;
  ...
  function resolveFileUrl(href) {
    ...
    var match = url.match(/\/view\/upload\/([^?#]+)/);
    if (match) {
      try {
        return normalizeUrl('/static/uploads/' + decodeURIComponent(match[1]));
  ...
  function isViewableUpload(href) {
    ...
    if (VIEW_UPLOAD_PATH.test(path)) {
      return PDF_EXT.test(url) || IMAGE_EXT.test(url) || true;
    }
  ...
  function open(url, title) {
    var rawUrl = normalizeUrl(url);
    if (!ensureModal()) {
      global.location.href = rawUrl;
      return;
    }
    var safeUrl = resolveFileUrl(rawUrl);
    ...
    if (openEl) openEl.href = rawUrl.indexOf('/view/upload/') >= 0 ? rawUrl : safeUrl;
