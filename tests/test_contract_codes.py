"""اختبارات ترقيم العقود (بادئة حسب النوع + تجديد)."""
...
def test_contract_prefix_for_type():
    assert contract_prefix_for_type('عقد صيانة') == 'CN-'
    assert contract_prefix_for_type('عقد ضمان') == 'CN-'
    assert contract_prefix_for_type('عقد طوارئ') == 'CN-'
    assert contract_prefix_for_type('عقد تركيب') == 'CI-'
    assert contract_prefix_for_type('عقد تحديث') == 'CI-'
    assert contract_prefix_for_type('تحديث مصعد قائم') == 'CI-'
    assert contract_prefix_for_type('Installation Contract') == 'CI-'
...
def test_contract_base_code_strips_year():
    ...
    assert contract_base_code('CI-00003-2026') == 'CI-00003'
...
def test_renewal_contract_code_appends_year():
    ...
    assert renewal_contract_code('CI-00007', 2027) == 'CI-00007-2027'
...
def test_normalize_contract_code_keeps_renewal_year():
    from contract_codes import normalize_contract_code

    assert normalize_contract_code('CN-1') == 'CN-00001'
    assert normalize_contract_code('CN-00001') == 'CN-00001'
    assert normalize_contract_code('CN-00001-2026') == 'CN-00001-2026'
    assert normalize_contract_code('CN-87-2026-2') == 'CN-00087-2026-2'
    assert normalize_contract_code('CI-3/2026') == 'CI-00003-2026'
